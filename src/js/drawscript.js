...
function sendData(data, ids, userName, house)
	{
		...
		$(document).ready(function()
		{
			$.ajax({url: "php/update.php", method: "get", dataType: "json", data: {ids: result, location: data, user: userName},success: function(result)
			{
				var data = result;
				for(i in data)
				{
				...
				var id = data[i].ID;
				var packet = {id: id, location: location, house: house, date: date, time: time, colour: colour};
